...
public class UIPropList : MonoBehaviour,TaskMessenger,TaskProcessor<PropContent>
{
    ...
    //接收来自GameManager删除物体的调用
    public int DeleteProp(string deletePropName)
    {
        if (!isGettingProp)
        {

            //若删除的为当前选中的物体，先取消选中
            if (thePropDataManager.GetIsPropChosen() && (thePropDataManager.GetNowChosenProp() == deletePropName))
            {
                thePropDataManager.SetNowChosenState(false, "None");
                uiPropDic[deletePropName].ObjTrigger(NormalTriggers.cancelChosen);
                nowProp = null;

            }


            thePropDataManager.DeleteProp(deletePropName);
            Destroy(uiPropDic[deletePropName].gameObject);
            uiPropDic.Remove(deletePropName);
