...
public class PropDataManager : MonoBehaviour
{
    ...
    //测试代码
    [SerializeField] private InteractiveObj[] testPropPrefabs;
    ...
    public bool IsPropExist(string propName)
    {
        return nowProps.Contains(propName);
    }


    public void DeleteProp(string propName)
    {
        nowProps.Remove(propName);
    } 
    ...
    private void Awake()
    {
        ...
        //测试代码
        for (int i = 0; i < testPropPrefabs.Length; i++)
        {
            propPrefabDictionary.Add(testPropPrefabs[i].GetObjName(), testPropPrefabs[i].gameObject);
        }
    }
